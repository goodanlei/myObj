...
Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            ...
            if(isvoid.auto){
                Dates.creation([arr[0], arr[1]|0, arr[2]|0],0,1);
...
//返回最终日期
Dates.creation = function(ymd,obj,hide){
    ...
    if(!hide){
        Dates.close();
        typeof Dates.options.choose === 'function' && Dates.options.choose(getDates,obj); 
    }
};
...
Dates.events = function(){
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                Dates.stopmp(ev);
                Dates.creation([this.getAttribute('y')|0, this.getAttribute('m')|0, this.getAttribute('d')|0],this);
            }
            var arr = [];
            arr[0]=this.getAttribute('y')*1;
            arr[1]=this.getAttribute('m')*1;
            arr[2]=this.getAttribute('d')*1;
        	rili_D(arr);
        });
    });
