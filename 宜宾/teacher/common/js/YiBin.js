var YiBin={};  
//是否为ie浏览器  
YiBin.IsIE=!!document.all;  
//ie浏览器版本  
YiBin.IEVersion=(function(){if(!YiBin.IsIE)return -1;try{return parseFloat(/msie ([\d\.]+)/i.exec(navigator.userAgent)[1]);}catch(e){return -1;}})();  
//按id获取对象  
YiBin.$=function(Id,isFrame){var o;if("string"==typeof(Id))o= document.getElementById(Id);else if("object"==typeof(Id))o= Id;else return null;return isFrame?(YiBin.IsIE?frames[Id]:o.contentWindow):o;}  
//按标签名称获取对象  
//页面的高和宽******************************  
YiBin.isStrict=document.compatMode == "CSS1Compat";  
YiBin.BodyScale={x:0,y:0,tx:0,ty:0};//（x，y）：当前的浏览器容器大小  （tx，ty）：总的页面滚动宽度和高度 
YiBin.getClientHeight=function(){/*if(YiBin.IsIE)*/return YiBin.isStrict ? document.documentElement.clientHeight :document.body.clientHeight;/*else return self.innerHeight;*/}  
YiBin.getScrollHeight=function(){var h=!YiBin.isStrict?document.body.scrollHeight:document.documentElement.scrollHeight;return Math.max(h,this.getClientHeight());}  
YiBin.getHeight=function(full){return full?this.getScrollHeight():this.getClientHeight();}  
YiBin.getClientWidth=function(){/*if(YiBin.IsIE)*/return YiBin.isStrict?document.documentElement.clientWidth:document.body.clientWidth;/*else return self.innerWidth;*/}  
YiBin.getScrollWidth=function(){var w=!YiBin.isStrict?document.body.scrollWidth:document.documentElement.scrollWidth;return Math.max(w,this.getClientWidth());}  
YiBin.getWidth=function(full){return full?this.getScrollWidth():this.getClientWidth();}  
YiBin.initBodyScale=function(){YiBin.BodyScale.x=YiBin.getWidth(false);YiBin.BodyScale.y=YiBin.getHeight(false);YiBin.BodyScale.tx=YiBin.getWidth(true);YiBin.BodyScale.ty=YiBin.getHeight(true);} 
//页面的高和宽******************************  
YiBin.Msg={  
    INFO:'info',  
    ERROR:'error',  
    ...
    moveProcessbar:function(){  
      var o=YiBin.$('dvProcessbar'),w=o.style.width;  
      if(w=='')w=20;  
      else{  
        w=parseInt(w)+20;  
        if(w>100)w=0;  
    ...
    InitMsg:function(width){  
      ...
      this.lightBox.id="YiBinlightBox";  
      document.body.appendChild(this.lightBox);  
      if(YiBin.IsIE&&YiBin.IEVersion<7){//加iframe层修正ie6下无法遮盖住select的问题  
        this.lightBox.innerHTML+=ifStr;  
        this.dvMsgBox.innerHTML+=ifStr;  
      }  
      this.dvBottom=YiBin.$('dvMsgBottom');  
      this.dvBtns=YiBin.$('dvMsgBtns');  
      this.dvCT=YiBin.$('dvMsgCT');  
      this.dvTitle=YiBin.$('dvMsgTitle');  
      this.IsInit=true;  
    },  
    ...
    show:function(cfg){  
      ...
      //添加窗体大小改变监听  
      if(YiBin.IsIE)window.attachEvent("onresize",this.onResize);  
      ...
      if(cfg.wait){  
        ...
        this.timer=setInterval(function(){YiBin.Msg.moveProcessbar();},1000);  
      }  
      else{  
        //if(!cfg.icon)cfg.icon=YiBin.Msg.INFO;  
    ...
    hide:function(){  
      ...
      if(this.timer){clearInterval(this.timer);this.timer=null;}  
      if(YiBin.IsIE)window.detachEvent('onresize',this.onResize);  
      else window.removeEventListener('resize',this.onResize,false);  
    },  
    onResize:function(isResize){  
       if(isResize)YiBin.initBodyScale();  
       YiBin.Msg.lightBox.style.width=YiBin.BodyScale.tx+'px';  
       YiBin.Msg.lightBox.style.height=YiBin.BodyScale.ty+'px';  
       YiBin.Msg.dvMsgBox.style.top=240+'px';  
       YiBin.Msg.dvMsgBox.style.left=Math.floor((YiBin.BodyScale.x-YiBin.Msg.dvMsgBox.offsetWidth)/2)+'px';  
    }  
